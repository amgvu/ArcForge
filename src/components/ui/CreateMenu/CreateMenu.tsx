...
const DSCreateMenu: React.FC<DSCreateMenuProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <Combobox.Options 
          className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-2xl bg-neutral-800 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-hidden sm:text-sm"
        >
          {isLoading ? (
              ...
              {filteredArcs.map((arc) => (
                <Combobox.Option
                  ...
                  className={({ active }) => `
                    ...
                    ${active ? 'bg-zinc-800 rounded-2xl text-[#D7DADC]' : 'text-[#D7DADC]'}
                  `}
                >
                  {({ selected }) => (
